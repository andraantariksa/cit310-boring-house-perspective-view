Public Class Form1
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        setrow(T7, 1, 0, 1, 0, 0)
        setrow(T7, 2, 0, 0, 1, 0)
        setrow(T7, 3, 0, 0, (-COP(2) / (BP - COP(2))), 1)
        ...
        Transform(Vertex, vertexConnection, 10, 15, FP, BP, COP)
    ...
    Function Parametric(ByVal a As Double(), ByVal b As Double(), ByVal t As Double) As Double()
        Return New Double() {
            a(0) + (t * (b(0) - a(0))),
            a(1) + (t * (b(1) - a(1))),
            a(2) + (t * (b(2) - a(2)))
    ...
    Function ClipCyrusBeck(ByVal P1 As Double(), ByVal P2 As Double(), ByVal FP As Double, ByVal BP As Double, ByVal COP As Double()) As Double()()
        ...
        Dim planeNormals = New Double()() {normalFront, normalBack, normalLeft, normalRight, normalTop, normalBottom}
        ...
        Dim planeNormalsArbitraryPoint = New Double()() {pointOnPlaneFront, pointOnPlaneBack, pointOnPlaneLeft, pointOnPlaneRight, pointOnPlaneTop, pointOnPlaneBottom}
        ...
        For i = 0 To 5
            ...
            Else
                If dotProdP1 < 0.0 And dotProdP2 > 0.0 Then
                    ...
                    entering.Append(t)
                    P1res = Parametric(P1res, P2res, t)
                ElseIf dotProdP1 > 0.0 And dotProdP2 < 0.0 Then
                    Dim t = FindTCyrusBeck(P1res, P2res, arbitraryPoint, normal)
                    leaving.Append(t)
                    P2res = Parametric(P1res, P2res, t)
                ElseIf dotProdP1 = 0.0 Then
                    If dotProdP2 < 0.0 Then
                        Dim t = FindTCyrusBeck(P1res, P2res, arbitraryPoint, normal)
                        P2res = Parametric(P1res, P2res, t)
                    End If
                ElseIf dotProdP2 = 0.0 Then
                    If dotProdP1 < 0.0 Then
                        Dim t = FindTCyrusBeck(P1res, P2res, arbitraryPoint, normal)
                        P1res = Parametric(P1res, P2res, t)
                    End If
                End If
        ...
        If entering.Count = 0 Or leaving.Count = 0 Then
            Return New Double()() {P1res, P2res}
        Else
            Dim tEntering = entering.OrderByDescending(Function(m) m).First
            Dim tLeaving = entering.OrderByDescending(Function(m) m).Last

            If tEntering <= tLeaving Then
                Return New Double()() {P1res, P2res}
            ElseIf tEntering > tLeaving Then
    ...
    Sub Transform(ByVal p(,) As Double, ByVal vertexConnection(,) As Integer, ByVal pointTotal As Integer, ByVal linen As Integer, ByVal FP As Double, ByVal BP As Double, ByVal COP As Double())
        ...
        For i = 0 To linen - 1
            Dim resLines = ClipCyrusBeck(
                    New Double() {lines(i).Vertex1.X, lines(i).Vertex1.Y, lines(i).Vertex1.Z, lines(i).Vertex1.W},
                    New Double() {lines(i).Vertex2.X, lines(i).Vertex2.Y, lines(i).Vertex2.Z, lines(i).Vertex2.W},
                    FP,
                    BP,
                    COP)

            If resLines Is Nothing Then
                lines(i).Vertex1.X = Double.NaN
                lines(i).Vertex1.Y = Double.NaN
                lines(i).Vertex1.Z = Double.NaN

                lines(i).Vertex2.X = Double.NaN
                lines(i).Vertex2.Y = Double.NaN
                lines(i).Vertex2.Z = Double.NaN
            Else
        ...
        For i = 0 To lines.Count - 1
            ...
            If Double.IsNaN(v(0)) Or Double.IsInfinity(v(0)) Then
                Continue For
            End If
